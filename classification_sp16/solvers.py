...
class GradientDescentSolver(Solver):
    ...
    def get_updates_without_momentum(self, loss_tensor, param_vars):
        ...
        for param in range(0, len(param_vars)):
            ...
            new_param_tensor =  param_vars[param] - grad_tensors[param] * self.learning_rate
            updates.append((param_vars[param], new_param_tensor))
    ...
    def get_updates_with_momentum(self, loss_tensor, param_vars):
        ...
        for param in range(0, len(param_vars)):
            ...
            newVar = self.momentum * vel_vars[param] - grad_tensors[param] * self.learning_rate
            new_param_tensor = param_vars[param] + newVar
            updates.append((param_vars[param], new_param_tensor))
            updates.append((vel_vars[param], newVar))
...
class StochasticGradientDescentSolver(GradientDescentSolver):
    ...
    def solve(self, input_train_data, target_train_data, input_val_data, target_val_data, model, callback=None):
        ...
        trainingDataGenerator = MinibatchIndefinitelyGenerator(train_data,1 ,self.shuffle)
        validationDataGenerator = MinibatchIndefinitelyGenerator(val_data,1, self.shuffle)

        for iter_ in range(self.iterations):
            "*** YOUR CODE HERE ***"
            trainData = trainingDataGenerator.next()
            validData = validationDataGenerator.next()
...
class MinibatchStochasticGradientDescentSolver(GradientDescentSolver):
    ...
    def solve(self, input_train_data, target_train_data, input_val_data, target_val_data, model, callback=None):
        ...
        for iter_ in range(self.iterations):
            "*** YOUR CODE HERE ***"
            trainData = trainingDataGenerator.next()
            valData = validationDataGenerator.next()

            train_loss = session.run([loss_tensor] + update_ops, feed_dict={placeholders[0]: trainData[0], placeholders[1]: trainData[1] })[0]
            val_loss = session.run(loss_tensor,   feed_dict={placeholders[0]: valData[0], placeholders[1]: valData[1]})
            # train_loss should be the loss of this iteration using only the training data that was used for the updates
            # val_loss should be the loss of this iteration using the same amount of data used for the updates, but using the validation data instead
            train_losses.append(train_loss)
            val_losses.append(val_loss)
